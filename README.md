...
Credits are a prepaid service: once spent, they paid for something already delivered (a listing published, an ad shown, a feature unlocked), so a refund cannot claw back value the consumer plugin already gave the buyer. Every gateway refund follows one SDK-wide rule, the same for every consumer:
